	 ...
	 if(empty($GameName&&$GameEntry))
	  {
		  ...
		  $sql="INSERT INTO pubglite_matches(name,Date,Time,map,type,view,entry_fee) VALUES(:n,:d,:t,:map,:type,:view,:fee)";
		  ...
		  if($Execute){
			  ...
			  $_SESSION["SuccessMessage"]="Match" .$laid. "Added Succussfully";
              if(!empty($laid))
              {   $temp="pubglite";
              $name=$temp.$laid;
              $sq="CREATE TABLE $name(sr_id int(2) auto_increment,user_game_name varchar(50),kills int(2),earning int(2),user_id int(2),PRIMARY KEY (sr_id),
    FOREIGN KEY (user_id) REFERENCES user(id) ON DELETE CASCADE )";
              $connectingdb->exec($sq);
...
<!--main area-->
  ...
  <div class="container app-stats">
    ...
    <h2 class="app-stats text-center" style="color:black;font-family: 'Times New Roman', Times, serif;"><span><i class="fas fa-plus mr-2"></i></span>ADD PUBG LITE MATCHES</h2>   
  </div>
</div> 
        ...
        <div class="row py-2" style="background-color:#92939E;">
            <div class="offset-lg-2 col-lg-8">
                <form method="POST">
                 ...
                 <option>Varenga</option>
                 <option>Golden Woods</option>
                 </select>
                 </div>
                 <div class="form-group text-white">
